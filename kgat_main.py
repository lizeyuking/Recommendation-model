...
if __name__ == '__main__':
    ...
    np.random.seed(2019)
    #args = parse_args() #命令行选项、参数和子命令解析器                          #!!!!!!!!!!
    ...
    *********************************************************
    ...
    if args.pretrain == 1:  # pre_model = 'mf'
        if args.model_type in ['bprmf', 'cke', 'fm', 'cfkg']:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if args.report != 1:
                ...
                # *********************************************************
                # 保存mf的预训练模型参数(即仅用户和项目嵌入)，用于预训练其他模型。
                if args.save_flag == -1:
                ...
                # *********************************************************
                # 保存kgat的预训练模型参数(即user & item & kg embeddings)，用于预训练其他模型。
                if args.save_flag == -2:
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        *********************************************************
        ...
        ... phase 1: to train the recommender.（CF部分损失函数Lcf的训练）
        """
        ...
        *********************************************************
        ...
        if args.model_type in ['kgat']:
            ...
            if args.use_att is True:  # 如果args.useatt参数设为true，则使用注意力机制，每次训练会更新A_in矩阵的值。
        ...
        *********************************************************
        ...
        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f], recall=[%.5f, %.5f], ' \
                       ...
                       (epoch, t2 - t1, t3 - t2, loss, base_loss, kge_loss, reg_loss, ret['recall'][0],
                        ret['recall'][-1],
    ...
    f.write(
        ...
        % (args.embed_size, args.lr, args.layer_size, args.node_dropout, args.mess_dropout, args.regs, args.adj_type,
           args.use_att, args.use_kge, args.pretrain, final_perf))
    f.close()
"""
python Main.py --model_type kgat --alg_type bi --dataset yelp2018 --regs [1e-5,1e-5] --layer_size [64,32,16] --embed_size 64 --lr 0.0001 --epoch 1000 --verbose 50 --save_flag 1 --pretrain -1 --batch_size 1024 --node_dropout [0.1] --mess_dropout [0.1,0.1,0.1] --use_att True --use_kge True
