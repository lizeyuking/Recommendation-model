...
class KGAT(object):
    def __init__(self, data_config, pretrain_data, args):
        ...
        '''
        *********************************************************
        创建占位符 for Input Data & Dropout.
        '''
        self._build_inputs()

        """
        *********************************************************
        创建模型参数 for CF & KGE parts.
        """
        self.weights = self._build_weights()

        """
        *********************************************************
        通过图神经网络的消息传递机制计算基于图的所有用户、项目和KG实体的表示。
        不同的三种聚合器:
            1. bi:双交互聚合器，考虑eh和eNh之间的两种特征交互 defined in 'KGAT: Knowledge Graph Attention Network for Recommendation', KDD2019;
            2. gcn:GCN聚合器[17]将两个表示相加并应用非线性变换  defined in 'Semi-Supervised Classification with Graph Convolutional Networks', ICLR2018;
            3. graphsage: GraphSage聚合器[9]拼接两个表示，然后是非线性转换 defined in 'Inductive Representation Learning on Large Graphs', NeurIPS2017.
        """
        #根据选择的聚合器去聚合向量
    ...
    def _parse_args(self, data_config, pretrain_data, args):
        # 解析参数
        self.model_type = 'kgat'  # 定义模型名称

        self.pretrain_data = pretrain_data  # 导入预训练的数据

        self.n_users = data_config['n_users']  # 用户
        self.n_items = data_config['n_items']  # 项目
        self.n_entities = data_config['n_entities']  # 知识图的实体
        self.n_relations = data_config['n_relations']  # 知识图的关系/边
        ...
        self.all_h_list = data_config['all_h_list']     #此时这些列表都还是初始值，不具有意义
        ...
        self.kge_dim = args.kge_size  # kge的维度
    ...
    def _build_inputs(self):
        ...
        self.h = tf.placeholder(tf.int32, shape=[None], name='h')       #形参，用于定义过程，在执行的时候再赋具体的值,shape=[None]为形状先不确定
        self.r = tf.placeholder(tf.int32, shape=[None], name='r')
        self.pos_t = tf.placeholder(tf.int32, shape=[None], name='pos_t')
        self.neg_t = tf.placeholder(tf.int32, shape=[None], name='neg_t')

        # Dropout:节点Dropout (self -networks采用);
        # 消息Dropout(在卷积操作上采用)。
    ...
    def _build_weights(self):
        ...
        initializer = tf.contrib.layers.xavier_initializer()        #高斯分布初始化权重矩阵，使得每一层输出的方差应该尽量相等
        ...
        else:                                                                                                               #嵌入用户、项目、实体的embedding    #####！！！
            all_weights['user_embed'] = tf.Variable(initial_value=self.pretrain_data['user_embed'], trainable=True,         #图变量的初始化
                                                    name='user_embed', dtype=tf.float32)

            item_embed = self.pretrain_data['item_embed']
            #n_entities = n_items + 辅助信息节点（如item为书本，则可以是书的作者）
            other_embed = initializer([self.n_entities - self.n_items, self.emb_dim])                                 #将所有项目之外的辅助信息进行高斯初始化

            all_weights['entity_embed'] = tf.Variable(initial_value=tf.concat([item_embed, other_embed], 0),            #将向量化后的项目节点矩阵和实体节点矩阵两个矩阵沿列进行合并，成为大实体矩阵
        ...
        for k in range(self.n_layers):      #？？对每层的节点权值进行初始化
    ...
    def _build_model_phase_I(self):
        if self.alg_type in ['bi', 'kgat']:
            self.ua_embeddings, self.ea_embeddings = self._create_bi_interaction_embed()    #self.ua_embeddings, self.ea_embeddings：分别是最终的user和item向量
        ...
        self.u_e = tf.nn.embedding_lookup(self.ua_embeddings, self.users)   #选取一个张量里面索引对应的元素
    ...
    def _build_model_phase_II(self):  # 这部分要实现的就是知识图嵌入
    ...
    def _get_kg_inference(self, h, r, pos_t, neg_t):  # 就是将h 和 t 投影到 关系r 空间中。
        embeddings = tf.concat([self.weights['user_embed'], self.weights['entity_embed']], axis=0)
        embeddings = tf.expand_dims(embeddings, 1)

        # 头和尾实体嵌入: batch_size *1 * emb_dim
    ...
    def _build_loss_phase_I(self):
        ...
        # 使用softplus作为BPR的损失函数，来避免nan错误的出现
        ...
        # 优化过程.RMSPropOptimizer
        self.opt = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss)

    def _build_loss_phase_II(self):  # 实现损失函数g(h,r,t)的计算
    ...
    def _generate_transE_score(self, h, t, r):  # 实现π（h,r,t）注意力分数的计算，用于后面实现注意力机制
        embeddings = tf.concat([self.weights['user_embed'], self.weights['entity_embed']], axis=0)
        embeddings = tf.expand_dims(embeddings, 1)
    ...
    def _statistics_params(self):   #统计参数数量
        # number of params
        total_parameters = 0
